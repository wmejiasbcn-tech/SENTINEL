...
def build_final_state(case: dict[str, Any], *, bypass_attempt: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    digest = hashlib.sha256(
        json.dumps({k: body[k] for k in REQUIRED_FIELDS if k != "gate_ref"}, sort_keys=True, ensure_ascii=False).encode("utf-8")
    ).hexdigest()
    body["gate_ref"] = f"gate:v1.0:{digest[:16]}"

    for field in REQUIRED_FIELDS:
        if field not in body:
            raise RuntimeError(f"final-state missing required field: {field}")
